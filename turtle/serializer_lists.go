...
func (ts *turtleState) isListCellShape(k string) bool {
...
func (ts *turtleState) detectLists() {
	...
	for k := range ts.spoMap {
		...
		for node := k; ; {
			...
			node = termKey(ts.spoMap[node][ts.restKey][0])
		}
		for _, n := range path {
			verdict[n] = result
		}
...
func (ts *turtleState) canWriteList(k string) bool {
	for node := k; node != ts.nilKey; node = termKey(ts.spoMap[node][ts.restKey][0]) {
